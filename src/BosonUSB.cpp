...
/* ---------------------------- Main Function ---------------------------------------*/
// ENTRY POINT
v4l2_buffer func(int argc, char** argv )
{
	// int ret;
	// int fd;
	int i;
	...
	// Boson320 , Boson 640
	thermal16=Mat(height, width, CV_16U, buffer_start);   // OpenCV input buffer  : Asking for all info: two bytes per pixel (RAW16)  RAW16 mode`
	thermal16_linear=Mat(height,width, CV_8U, 1);         // OpenCV output buffer : Data used to display the video
...
// void show(v4l2_buffer bufferinfo){
...
// 	EXIT_SUCCESS;
// }
Mat show(v4l2_buffer& bufferinfo, Mat& thermal16, Mat& thermal16_linear, int height, int width, int zoom_enable, int record_enable, const char* thermal_sensor_name, long& frame) {
    // 画面表示のためのループ
    ...
    if (video_mode == RAW16) {
        ...
        if (zoom_enable == 0) {
            ...
            resize(thermal16_linear, thermal16_linear_zoom, size);
            sprintf(label, "%s : RAW16 Zoom", thermal_sensor_name);
            // imshow(label, thermal16_linear_zoom);
        }
...
//     // funcに新しい引数を渡す
//     v4l2_buffer k =func(3, new_argv); // argcは3に設定
//     Mat thermo = show(k, thermal16, thermal16_linear, height, width,  zoom_enable, record_enable,  thermal_sensor_name, frame);
//     imshow("aa",thermo);
//     waitKey(0);
//     return 0;
// }